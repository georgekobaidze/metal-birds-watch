...
function setupInfoHandlers() {
  ...
  // Contribute link in What's Next modal
  const roadmapContributeLink = document.getElementById('roadmap-contribute-link');
  if (roadmapContributeLink) {
    roadmapContributeLink.addEventListener('click', (e) => {
      e.preventDefault();
      ...
      // Wait for close animation, then open Contribute modal
      setTimeout(() => {
        showContributeModal();
      }, 350);
    });
...
function showContributeModal() {
  ...
  showModal('Contribute', content, null, null);
  
  // Add handler for social link
  setTimeout(() => {
    const socialLink = document.getElementById('contribute-social-link');
    if (socialLink) {
